...
def get_geofences_from_db(dId):
    ...
    if "config" not in data or data["config"]=="": return []
    if "geofences" not in data["config"]:return []
...
def verify_geofences_general(device_data, current_coords):
    ...
    if len(geofences_array)==0: return
...
# print(requestDatas()[0])


def count_exits(all_entrances):
    # print(all_entrances[0])
    full_response={}
    for entrance in all_entrances:
        ...
        verify_geofences_general(full_response[entrance["dId"]], [entrance["lat"], entrance["lng"]])
    ...
    for response in full_response:
        # print(response)
        min_response["detail"].append({"dId":response, "exits_count":full_response[response]["exits_count"]})
        min_response["exits_count"]=min_response["exits_count"]+full_response[response]["exits_count"]

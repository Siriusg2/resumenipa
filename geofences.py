...
def crear_geocerca(coordenadas_geocerca=None, diametro_metros=100):
    if isinstance(coordenadas_geocerca, list) and len(coordenadas_geocerca) == 2:
        ...
        circunference = center_point.buffer(radio_distance / 100000)  # Buffer en metros
...
def verify_geofences_general(device_data, current_coords):
    ...
    for geofence in geofences_array:
        geofence_obj = crear_geocerca(geofence["coords"], geofence["diameter"])
        ...
        if (
            verify_geofence_in_out(geofence_obj, current_coords) == True
            and device_data["last_geofence"] != geofence["name"]
        ):
        ...
        # # Corrobora si el dispositivo salió de una geocerca (está fuera de la geocerca y antes estaba dentro)
            ...
            device_data["last_geofence"] == geofence["name"]
            and verify_geofence_in_out(geofence_obj, current_coords) == False
        ):
...
def count_exits(all_entrances):
    ...
    for entrance in all_entrances:
        ...
        verify_geofences_general(
            full_response[entrance["dId"]], [entrance["lat"], entrance["lng"]]
        )

    min_response = {"exits_count": 0, "detail": []}
    for response in full_response:
        min_response["detail"].append(
            {
                "dId": response,
                "name": full_response[response]["name"],
                "exits_count": full_response[response]["exits_count"],
            }
        )
        min_response["exits_count"] = (
            min_response["exits_count"] + full_response[response]["exits_count"]
        )

    return (
        min_response if len(min_response["detail"]) > 1 else min_response["detail"][0]
    )
...
def create_excel_file():
    data = count_exits(all_data)
    df = pd.DataFrame(data["detail"])
    excel_file = "excel_reports/geofences.xlsx"
